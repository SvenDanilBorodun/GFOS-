...
# UNIVERSAL VERSION - Auto-detects installed tools
...
function Find-JavaHome {
    ...
    # 2. Check Windows Registry for Eclipse Adoptium (Temurin)
    $adoptiumPaths = @(
        ...
        if (Test-Path $regPath) {
            ...
            foreach ($ver in $versions) {
                ...
                if (Test-Path $hotspotPath) {
                    $javaPath = (Get-ItemProperty $hotspotPath -ErrorAction SilentlyContinue).Path
    ...
    if ($javaCmd) {
        ...
        # java.exe is typically in bin folder, go up two levels for JAVA_HOME
...
function Find-MavenHome {
    # 1. Check existing MAVEN_HOME or M2_HOME environment variable
    if ($env:MAVEN_HOME -and (Test-Path "$env:MAVEN_HOME\bin\mvn.cmd")) {
        return $env:MAVEN_HOME
    }
    if ($env:M2_HOME -and (Test-Path "$env:M2_HOME\bin\mvn.cmd")) {
        return $env:M2_HOME
    }
    ...
    foreach ($pattern in $commonPaths) {
        $found = Get-Item $pattern -ErrorAction SilentlyContinue | Sort-Object Name -Descending | Select-Object -First 1
        if ($found -and (Test-Path "$($found.FullName)\bin\mvn.cmd")) {
            return $found.FullName
        }
    }

    # 3. Try to find mvn in PATH and derive MAVEN_HOME
    $mvnCmd = Get-Command mvn -ErrorAction SilentlyContinue
    if ($mvnCmd) {
        $mvnExe = $mvnCmd.Source
        $binDir = Split-Path $mvnExe -Parent
        $mavenHome = Split-Path $binDir -Parent
...
function Find-GlassFishHome {
    # 1. Check existing GLASSFISH_HOME environment variable
    if ($env:GLASSFISH_HOME -and (Test-Path "$env:GLASSFISH_HOME\bin\asadmin.bat")) {
        return $env:GLASSFISH_HOME
    }

    # 2. Check common installation paths (GlassFish has nested structure)
    $basePaths = @(
        "C:\glassfish*",
    ...
    foreach ($pattern in $basePaths) {
        ...
        foreach ($baseDir in $found) {
            # Check for nested glassfish7, glassfish6, etc.
            $nestedDirs = Get-ChildItem $baseDir.FullName -Directory -Filter "glassfish*" -ErrorAction SilentlyContinue
            foreach ($nested in $nestedDirs) {
                if (Test-Path "$($nested.FullName)\bin\asadmin.bat") {
                    return $nested.FullName
                }
            }
            # Check if asadmin is directly in base/bin
            if (Test-Path "$($baseDir.FullName)\bin\asadmin.bat") {
                return $baseDir.FullName
            }
        }
    }

    # 3. Try to find asadmin in PATH
    $asadminCmd = Get-Command asadmin -ErrorAction SilentlyContinue
    if ($asadminCmd) {
        $asadminExe = $asadminCmd.Source
        $binDir = Split-Path $asadminExe -Parent
        $glassfishHome = Split-Path $binDir -Parent
        if (Test-Path "$glassfishHome\bin\asadmin.bat") {
...
function Find-PostgresBin {
    ...
    if ($env:POSTGRES_BIN -and (Test-Path "$env:POSTGRES_BIN\psql.exe")) {
        return $env:POSTGRES_BIN
    }
    if ($env:PGBIN -and (Test-Path "$env:PGBIN\psql.exe")) {
        return $env:PGBIN
    }

    # 2. Check common installation paths (get highest version)
    $pgBasePaths = @(
        "C:\Program Files\PostgreSQL",
        "C:\PostgreSQL"
    )
    foreach ($basePath in $pgBasePaths) {
    ...
    # 3. Try to find psql in PATH
    $psqlCmd = Get-Command psql -ErrorAction SilentlyContinue
    if ($psqlCmd) {
        $psqlExe = $psqlCmd.Source
        $binDir = Split-Path $psqlExe -Parent
        if (Test-Path "$binDir\psql.exe") {
            return $binDir
        }
    }
...
function Get-PostgresAdminPassword {
    # Check if already cached in script scope
    if ($script:CachedPgPassword) {
        return $script:CachedPgPassword
    }

    # Check environment variable
    if ($env:PGPASSWORD) {
        $script:CachedPgPassword = $env:PGPASSWORD
        return $env:PGPASSWORD
    }

    # Prompt user
    Write-Host ""
    Write-Host "PostgreSQL admin password required." -ForegroundColor Yellow
    Write-Host "Tip: Set PGPASSWORD environment variable to avoid this prompt." -ForegroundColor Gray
    $securePassword = Read-Host "Enter password for PostgreSQL user 'postgres'" -AsSecureString
    $password = [Runtime.InteropServices.Marshal]::PtrToStringAuto(
        [Runtime.InteropServices.Marshal]::SecureStringToBSTR($securePassword))
    $script:CachedPgPassword = $password
    return $password
}
...
$CONFIG = @{
    ...
    MAVEN_HOME = Find-MavenHome
    GLASSFISH_HOME = Find-GlassFishHome
    POSTGRES_BIN = Find-PostgresBin
    NODE_PATH = $null  # Wird automatisch erkannt via PATH
    ...
    POSTGRES_ADMIN_PASSWORD = $null  # Will be prompted when needed

    # Projektpfade (relativ zum Skript-Speicherort)
    PROJECT_ROOT = $PSScriptRoot
    BACKEND_PATH = Join-Path $PSScriptRoot "backend"
    FRONTEND_PATH = Join-Path $PSScriptRoot "frontend"
    DATABASE_PATH = Join-Path $PSScriptRoot "database"
...
GFOS Digital Ideen-Board - Startskript (Universal Version)
============================================================

Verwendung: .\start.ps1 [Optionen]

Optionen:
  -SkipChecks     Ueberspringe Abhaengigkeitspruefungen (schnellerer Start)
  -SkipBuild      Ueberspringe Maven Build (verwende bestehendes WAR)
...
Auto-Erkennung:
  Das Skript erkennt automatisch installierte Tools in gaengigen Verzeichnissen.
  Optional koennen Sie Umgebungsvariablen setzen um die Erkennung zu ueberschreiben:
    - JAVA_HOME      (z.B. C:\Program Files\Eclipse Adoptium\jdk-17.0.x)
    - MAVEN_HOME     (z.B. C:\apache-maven-3.9.x)
    - GLASSFISH_HOME (z.B. C:\glassfish-7.x.x\glassfish7)
    - POSTGRES_BIN   (z.B. C:\Program Files\PostgreSQL\17\bin)
    - PGPASSWORD     (PostgreSQL Admin-Passwort, um Abfrage zu vermeiden)

Beispiele:
  .\start.ps1                    # Vollstaendiger Start mit allen Ueberpruefungen
  .\start.ps1 -SkipChecks        # Ueberspringe Abhaengigkeitsverifikation
  .\start.ps1 -FrontendOnly      # Starten Sie nur Frontend
...
function Test-MavenInstallation {
    ...
    3. The script will auto-detect Maven, or set MAVEN_HOME environment variable

    Auto-detection searched: C:\apache-maven-*, C:\Program Files\Apache\maven*
...
function Test-PostgresInstallation {
    ...
    4. The script will auto-detect PostgreSQL, or set POSTGRES_BIN environment variable

    Auto-detection searched: C:\Program Files\PostgreSQL\*\bin
...
function Test-GlassFishInstallation {
    ...
    2. Extract to C:\glassfish-7.x.x
    3. The script will auto-detect GlassFish, or set GLASSFISH_HOME environment variable

    Auto-detection searched: C:\glassfish*\glassfish*, C:\Program Files\glassfish*
...
function Initialize-Database {
    ...
    # Get admin password (prompts if not set)
    ...
    # Grant sequence permissions to prevent permission errors
    Write-Info "Granting sequence permissions to $($CONFIG.DB_USER)..."
    $env:PGPASSWORD = $adminPassword
    & $psqlPath -U $CONFIG.POSTGRES_ADMIN_USER -h $CONFIG.DB_HOST -p $CONFIG.DB_PORT -d $CONFIG.DB_NAME -c "GRANT USAGE, SELECT, UPDATE ON ALL SEQUENCES IN SCHEMA public TO $($CONFIG.DB_USER);" 2>&1 | Out-Null

    if ($LASTEXITCODE -eq 0) {
        Write-Success "Sequence permissions granted"
    } else {
        Write-Warning "Could not grant sequence permissions (may already exist)"
    }

    # Grant table permissions for full access
    Write-Info "Granting table permissions to $($CONFIG.DB_USER)..."
    & $psqlPath -U $CONFIG.POSTGRES_ADMIN_USER -h $CONFIG.DB_HOST -p $CONFIG.DB_PORT -d $CONFIG.DB_NAME -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO $($CONFIG.DB_USER);" 2>&1 | Out-Null

    if ($LASTEXITCODE -eq 0) {
        Write-Success "Table permissions granted"
    } else {
        Write-Warning "Could not grant table permissions (may already exist)"
    }

    # Set default privileges for future objects
    Write-Info "Setting default privileges for future objects..."
    & $psqlPath -U $CONFIG.POSTGRES_ADMIN_USER -h $CONFIG.DB_HOST -p $CONFIG.DB_PORT -d $CONFIG.DB_NAME -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO $($CONFIG.DB_USER);" 2>&1 | Out-Null
    & $psqlPath -U $CONFIG.POSTGRES_ADMIN_USER -h $CONFIG.DB_HOST -p $CONFIG.DB_PORT -d $CONFIG.DB_NAME -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO $($CONFIG.DB_USER);" 2>&1 | Out-Null

    if ($LASTEXITCODE -eq 0) {
        Write-Success "Default privileges configured"
    } else {
        Write-Warning "Could not set default privileges"
    }
...
function Show-DetectedConfig {
    Write-Header "Auto-Detected Configuration"

    # Show detected paths with status
    if ($CONFIG.JAVA_HOME) {
        Write-Success "Java:      $($CONFIG.JAVA_HOME)"
    } else {
        Write-Error "Java:      NOT FOUND"
    }

    if ($CONFIG.MAVEN_HOME) {
        Write-Success "Maven:     $($CONFIG.MAVEN_HOME)"
    } else {
        Write-Error "Maven:     NOT FOUND"
    }

    if ($CONFIG.GLASSFISH_HOME) {
        Write-Success "GlassFish: $($CONFIG.GLASSFISH_HOME)"
    } else {
        Write-Error "GlassFish: NOT FOUND"
    }

    if ($CONFIG.POSTGRES_BIN) {
        Write-Success "PostgreSQL: $($CONFIG.POSTGRES_BIN)"
    } else {
        Write-Error "PostgreSQL: NOT FOUND"
    }

    Write-Info "Project:   $($CONFIG.PROJECT_ROOT)"
    Write-Host ""
}
